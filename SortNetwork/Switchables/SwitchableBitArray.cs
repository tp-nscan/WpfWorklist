...
{
    public interface ISwitchableBitArray : IEquatable<ISwitchableBitArray>, ISwitchable
    {
    ...
    public static class SwitchableBitArray 
    {
        ...
        public static IEnumerable<ISwitchableBitArray> MakeRandoms(int keyCount, IRando random, int itemCount)
        {
            for (var i = 0; i < itemCount; i++)
            {
                yield return Make(Generators.RandomBits(keyCount, random.ToBool(), 0.5).Take(keyCount), keyCount);
            }
        }
    ...
    class SwitchableBitArrayImpl : ISwitchableBitArray
    {
        ...
        public string StringValue
        {
            get
            {
               return _bitValues.Aggregate
                    (
                        string.Empty,
                        (s, bv) => s  + (string.IsNullOrEmpty(s) ? string.Empty :  ", ") + (bv ? "T" : "F")
                    );
            }
        }
        ...
        public bool Equals(ISwitchableBitArray other)
        {
        ...
        public override bool Equals(object obj)
        {
            ...
            return Equals((ISwitchableBitArray)obj);
        }
        ...
        public override int GetHashCode()
        {
            unchecked
            {
                return Values.Aggregate(1, (current, value) => (current*397) ^ (value ? 3 : 1));
