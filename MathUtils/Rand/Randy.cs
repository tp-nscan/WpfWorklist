...
namespace MathUtils.Rand
{
    public static class Randy
    {
        ...
        public static IRandomBool ToBool(this IRandoCore rando)
        {
            return new RandomBool(rando.Rando, 0.5);
        }

        public static IRandomBool ToBool(this IRandoCore rando, double trueProbability)
        {
            return new RandomBool(rando.Rando, trueProbability);
        }

        public static IRandomInt ToInt(this IRandoCore rando)
        {
            return new RandomInt(rando.Rando, int.MaxValue - 1);
        }

        public static IRandomInt ToInt(this IRandoCore rando, int maxVal)
        {
            return new RandomInt(rando.Rando, maxVal);
        }

        public static IRandomDouble ToDouble(this IRandoCore rando)
        {
            return new RandomDouble(rando.Rando);
        }

        public static IRandomGuid ToGuid(this IRandoCore rando)
        {
            return new RandomGuid(rando.Rando);
        }

        public static IGenerator<bool> MakeFixedBools(IEnumerable<bool> values)
        {
            return new FixedBools(values);
        }

        public static IGenerator<int> MakeFixedInts(IEnumerable<int> values)
        {
            return new FixedInts(values);
        }
    }

    class RandomBool : IRandomBool
    {
        public RandomBool(IRando rando, double trueProbability)
        {
            _rando = rando;
        ...
        public bool Next()
        {
            return Rando.NextDouble() < _trueProbability;
        }

        public NumberGeneratorType NumberGeneratorType
        {
            get { return Rando.NumberGeneratorType; }
        }

        public int Seed
        {
            get { return Rando.Seed; }
        }

        public long UseCount { get { return Rando.UseCount; } }

        public bool Next(double trueProbability)
        {
            return Rando.NextDouble() < trueProbability;
        }
    ...
    class RandomInt : IRandomInt
    {
        public RandomInt(IRando rando, int maxVal)
        {
            _rando = rando;
            if (maxVal < 0 || maxVal == int.MaxValue)
            {
                throw new ArgumentException("Maxval is less than zero or equal to int.MaxValue");
            }
            _maxVal = maxVal;
            _genCap = (int.MaxValue - 1) - ((int.MaxValue - 1) % MaxVal);
        }

        private readonly int _genCap;
        ...
        public int Next()
        {
            var randVal = Rando.NextInt();
            while (randVal > _genCap)
            {
                randVal = Rando.NextInt();
            }
        ...
        {
            var genCap = (int.MaxValue - 1) - ((int.MaxValue - 1) % maxVal);

            var randVal = Rando.NextInt();
            while (randVal > genCap)
            {
                randVal = Rando.NextInt();
            }
            return randVal % maxVal;
    ...
    class RandomGuid : IRandomGuid
    {
        public RandomGuid(IRando rando)
        {
            _rando = rando;
        }
    ...
    class RandomDouble : IRandomDouble
    {
        public RandomDouble(IRando rando)
        {
            _rando = rando;
        }
    ...
    class RandoReg : IRando
    {
        ...
        public NumberGeneratorType NumberGeneratorType
        {
            get { return NumberGeneratorType.RandomDotNet; }
        }
    ...
    class RandoFast : IRando
    {
        private readonly FastRandom _random;
        public RandoFast(int seed)
        {
            _seed = seed;
        ...
        public NumberGeneratorType NumberGeneratorType
        {
            get { return NumberGeneratorType.RandomFast; }
        }
    ...
    class FixedBools : IGenerator<bool>
    {
        private readonly List<bool> _rands;

        public FixedBools(IEnumerable<bool> randBools)
        {
            _rands = randBools.ToList();
        }
        ...
        public bool Next()
        {
            return _rands[(int) _useCount++];
        }

        public NumberGeneratorType NumberGeneratorType
        {
            get {return NumberGeneratorType.NonRandom;}
        }

        public int Seed { get { return 0; } }
    ...
    class FixedInts : IGenerator<int>
    {
        private readonly List<int> _rands;

        public FixedInts(IEnumerable<int> randInts)
        {
            _rands = randInts.ToList();
        }
        ...
        public NumberGeneratorType NumberGeneratorType { get { return NumberGeneratorType.NonRandom;} }
...
// ENHANCEMENT: Replace usages of this class with the superceding version from Math.Net.
    ...
    public class FastRandom
    {
        ...
        const double RealUnitUint = 1.0/(uint.MaxValue+1.0);
        ...
        public int Next(int lowerBound, int upperBound)
        {
            ...
            {   // If range is <0 then an overflow has occured and must resort to using long integer arithmetic instead (slower).
                ...
                return lowerBound+(int)((RealUnitUint*(_w=(_w^(_w>>19))^(t^(t>>8))))*(upperBound-(long)lowerBound));
            }
