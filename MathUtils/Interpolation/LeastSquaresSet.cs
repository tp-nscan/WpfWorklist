...
{
    public interface ILeastSquaresSet
    {
        double RSquare { get; }
        double Slope { get; }
        double XIntercept { get; }
        double YIntercept { get; }
        double ComputeRSquared();
    }

    public class LeastSquaresSet : IList<Point>, ILeastSquaresSet
    {
        private readonly List<Point> _internalList = new List<Point>();

        public LeastSquaresSet(IEnumerable<Point> points)
        {
            ResetValues();
