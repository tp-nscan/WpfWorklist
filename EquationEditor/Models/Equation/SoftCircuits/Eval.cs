...
namespace EquationEditor.Models.Equation.SoftCircuits
{
    ...
    {
        public TextParser(string exp)
        {
            _chars = exp.ToCharArray();
        }

        private char[] _chars;

        public bool EndOfText { get { return Position == _chars.GetLength(0) - 1; } }
        ...
        {
            while (char.IsWhiteSpace(_chars[Position]) )
            {
                MoveAhead();
            }
        }
        ...
        public string Extract(int start, int position)
        {
